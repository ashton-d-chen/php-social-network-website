...
FOTO.Slider = {
	...
	size : { },		// a hash for storing size information of thumbnail
	...
	buildThumbBar: function(key,bar){
		...
		for(i in this.bucket[key]){
			...
			var width = this.size[key][i];  // get the width of thumbnail
			div.css('width', width + 'px');  // set css of thumbnail.
			...
			if (this.data[key]['thumbWidth'] > maxWidth)
				maxWidth = this.data[key]['thumbWidth'];
		}
		...
		$(bar).css('width',((slot+1)*maxWidth)+'px')
