	...
	class YoutubeSidebarWidget extends WP_Widget {
		...
		function widget($args, $instance) {
			...
			if($ident_type == "username") {
				if($simple_xml = simplexml_load_file($username_url)) {
					$youtube_is_up = true;
				} else {
					$youtube_is_up = false;
				}
						...
						if($fil) {
							if(strpos($item->description, $fil)) {
								if($i <= $num) {
									...
									if($titles == "yes") {
										$output .= "<li id='$hash[0]'><img src='$img' alt='$item->title' desc='" . strip_tags($item->description) . "' /><div class='play_arrow'></div>\n\r<a href='#'>$item->title</a>";
									} else {
										$output .= "<li id='$hash[0]'><img src='$img' alt='$item->title' desc='" . strip_tags($item->description) . "' /><div class='play_arrow'></div>\n\r";
									}
				...
				if($simple_xml = simplexml_load_file($playlist_url)) {
					$youtube_is_up = true;
				} else {
					$youtube_is_up = false;
				}
					...
					foreach($simple_xml->entry as $item) {
						foreach($item->link as $link_url) {
							if($link_url['type'] == "text/html" && $link_url['rel'] == "alternate") {
								...
								if($titles == "yes") {
									$output .= "<li id='$hash[0]'><img src='$img' alt='$item->title' desc='$item->title' /><div class='play_arrow'></div>\n\r<a href='#'>$item->title</a>";
								} else {
									$output .= "<li id='$hash[0]'><img src='$img' alt='$item->title' desc='$item->title' /><div class='play_arrow'></div>\n\r";
								}
		...
		function form($instance) {				
			$instance = wp_parse_args( (array) $instance, array('title'=>'', 'ident_type'=>'username', 'ident'=>'', 'num_videos'=>'2', 'filter'=>'') );
